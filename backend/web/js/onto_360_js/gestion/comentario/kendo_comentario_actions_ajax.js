		    ...
		    grid.dataSource.query({
		        ...
		        filter: {
		            ...
		            filters: [
		                ...
		                {field: "fecha", operator: "contains", value: q},

		                {field: "usuario", operator: "contains", value: q},

						{field: "ontologia", operator: "contains", value: q}
...
$("#btn_modal_comentario").click(function() {
	Comentario.oldElement=null;
    $('#select2-id_ontologia-container').html('');
    Comentario.combobox_ontologia.append('<option selected="" value=""></option>').change();
    $('#select2-id_usuario-container').html('');
    Comentario.combobox_usuario.append('<option selected="" value=""></option>').change();
    $('#idcomentario').val('-1');
    Comentario.comentario_form.data('bootstrapValidator').resetForm(true);
    $('#comentario_form')[0].reset();
	 $('#btnaction_comentario').html('Guardar');
	 $("#title_comentario").html('Insertar Comentario');
	 $("#taskcomentario").val('insert');
	 $('#btnaction_comentario_new').show();
...
$("#reload_comentario").click(function() {
	if($('#taskcomentario').val()!='update') {
		...
		$("#comentario").val($.trim(Comentario.oldElement.comentario));
	 	Comentario.combobox_ontologia.append('<option selected="" value="'+Comentario.oldElement.id_ontologia+'">'+Comentario.oldElement.nombre+'</option>').change();
...
function showUpdate_comentario(e) {
    ...
    Comentario.comentario_form.data('bootstrapValidator').resetForm(true);
	 ...
	 $('#idcomentario').val(dataItem.idcomentario);
	$("#comentario").val($.trim(Comentario.oldElement.comentario));
	 	$("#fecha").val($.trim(Comentario.oldElement.fecha));
	 	Comentario.combobox_ontologia.append('<option selected="" value="'+Comentario.oldElement.id_ontologia+'">'+Comentario.oldElement.nombre+'</option>').change();
	 	Comentario.combobox_ontologia.val(Comentario.oldElement.id_ontologia);
	 	...
	 	Comentario.combobox_usuario.val(Comentario.oldElement.id_usuario);
	 $("#taskcomentario").val('update');
	 $('#btnaction_comentario_new').hide();
	 $('#btnaction_comentario').html('Actualizar');
	 $("#title_comentario").html('Actualizar Comentario');
	 $("#modal_comentario").modal();
	}
...
$('.btnaction').click(function () {
	 var btnid=this.id;
    Comentario.comentario_form.data('bootstrapValidator').validate();
        if(Comentario.comentario_form.data('bootstrapValidator').isValid())
			{
       		var comentario={};
				
       		comentario.comentario=$("#comentario").val();
       		comentario.fecha=$("#fecha").val();
       		comentario.id_ontologia=Comentario.combobox_ontologia.val();
       		comentario.id_usuario=Comentario.combobox_usuario.val();
       			var url=urlhome+"gestion/comentario/create";
					var title="Insertando elemento...";
        	if($('#taskcomentario').val()=="update")
        		{
            		var url=urlhome+"gestion/comentario/update";
					title="Actualizando elemento...";
					$('#btnaction_comentario_new').hide();
            		comentario.idcomentario=Comentario.oldElement.idcomentario;
        		
        		}
        		Noty('#2D8F3C', 'success', false, 'fa fa-spinner faa-spin animated', '', title,'center');
        		if(btnid.indexOf('new')!=-1 && $('#taskcomentario').val() != 'update')
         $.ajax({
            type: "POST",
            url:url,
			 data:{
            ...
            success:function(response){
                if(response.success)
                {
                    ...
                    Noty('#2D8F3C','success',2000,'fa fa-thumbs-o-up','El comentario fue '+accion+' correctamente', message,'topRight');
                }
                else
                {
...
function delete_element_comentariobyonto(e)
{
	var dataItem={};
	...
	$.MetroMessageBox({
			...
			if(a=="Aceptar")
			{
				$.ajax({
					type: "POST",
					url:urlhome+"/gestion/comentario/deletebyonto",
					data:{
						array:JSON.stringify(array),
						_onto_CSRF:_csrf
					},
