...
$("#btn_modal_respuesta").click(function() {
	Respuesta.oldElement=null;
    $('#idrespuesta').val('-1');
    Respuesta.respuesta_form.data('bootstrapValidator').resetForm(true);
    $('#respuesta_form')[0].reset();
	 $('#btnaction_respuesta').html('Guardar');
	 $("#title_respuesta").html('Insertar Respuesta');
	 $("#taskrespuesta").val('insert');
	 $('#btnaction_respuesta_new').show();
...
function showUpdate_respuesta(e) {

    Respuesta.respuesta_form.data('bootstrapValidator').resetForm(true);
	 var dataItem=Respuesta.finditem(e.id);
	 Respuesta.oldElement= dataItem;
	 $('#idrespuesta').val(dataItem.idrespuesta);
	 	$("#respuesta").val($.trim(Respuesta.oldElement.respuesta));
	 $("#taskrespuesta").val('update');
	 $('#btnaction_respuesta_new').hide();
	 $('#btnaction_respuesta').html('Actualizar');
	 $("#title_respuesta").html('Actualizar Respuesta');
	 $("#modal_respuesta").modal();
	}
...
$('.btnaction').click(function () {
	 var btnid=this.id;
    Respuesta.respuesta_form.data('bootstrapValidator').validate();
        if(Respuesta.respuesta_form.data('bootstrapValidator').isValid())
			{
       		var respuesta={};
       		respuesta.respuesta=$("#respuesta").val();
       			var url=urlhome+"gestion/respuesta/create";
					var title="Insertando elemento...";
        	if($('#taskrespuesta').val()=="update")
        		{
            		var url=urlhome+"gestion/respuesta/update";
					title="Actualizando elemento...";
					$('#btnaction_respuesta_new').hide();
            		respuesta.idrespuesta=Respuesta.oldElement.idrespuesta;
        		
        		}
        		Noty('#2D8F3C', 'success', false, 'fa fa-spinner faa-spin animated', '', title,'center');
        		if(btnid.indexOf('new')!=-1 && $('#taskrespuesta').val() != 'update')
         $.ajax({
            type: "POST",
            url:url,
            data:{
            ...
            success:function(response){
                if(response.success)
                {
                    ...
                    Noty('#2D8F3C','success',2000,'fa fa-thumbs-o-up','El respuesta fue '+accion+' correctamente', message,'topRight');
                }
                else
                {
