...
$("#btn_modal_cuestionario").click(function() {
	Cuestionario.oldElement=null;
	$('#select2-id_pregunta-container').html('');
	Cuestionario.combobox_pregunta.append('<option selected="" value=""></option>').change();
	$('#select2-id_respuesta-container').html('');
	$('.select2-selection__rendered').html('');
	// Cuestionario.combobox_respuesta.select2('destroy')
	// ComponentsCuestionario.initid_respuesta();
	Cuestionario.combobox_respuesta.append('<option selected="" value=""></option>').change();
	$('#idcuestionario').val('-1');
	Cuestionario.combobox_respuesta.val(null).trigger('change');
	Cuestionario.cuestionario_form.data('bootstrapValidator').resetForm(true);
	$('#cuestionario_form')[0].reset();
	$('#resp_correcta').iCheck('uncheck');
	$('#btnaction_cuestionario').html('Guardar');
	$("#title_cuestionario").html('Insertar Cuestionario');
	$("#taskcuestionario").val('insert');
	$('#btnaction_cuestionario_new').show();
	$('#select2-id_pregunta_combo-container').html('');
	$('#select2-id_respuesta_combo-container').html('');
	$("#modal_cuestionario").modal();
});
...
$("#reload_cuestionario").click(function() {
	if($('#taskcuestionario').val()!='update') {
		...
		Cuestionario.cuestionario_form.data('bootstrapValidator').resetForm(true);
		Cuestionario.combobox_respuesta.append('<option selected="" value="'+Cuestionario.oldElement.id_respuesta+'">'+Cuestionario.oldElement.respuesta+'</option>').change();
		Cuestionario.combobox_respuesta.val(Cuestionario.oldElement.id_respuesta);
		...
		$("#cuestionario").val($.trim(Cuestionario.oldElement.cuestionario));
...
function showUpdate_cuestionario(e) {

	Cuestionario.cuestionario_form.data('bootstrapValidator').resetForm(true);
	var dataItem=Cuestionario.finditem(e.id);
	Cuestionario.oldElement= dataItem;
	$('#resp_correcta').iCheck('uncheck');
	if(Cuestionario.oldElement.resp_correcta==1)
	{
	...
	arreglo.forEach(function(element,index){
		...
		Cuestionario.combobox_respuesta.append('<option selected="" value="'+resp.id_respuesta+'">'+resp.respuesta+'</option>').change();
	})
	$('#idcuestionario').val(dataItem.idcuestionario);
	Cuestionario.combobox_pregunta.append('<option selected="" value="'+Cuestionario.oldElement.id_pregunta+'">'+Cuestionario.oldElement.pregunta+'</option>').change();
	Cuestionario.combobox_pregunta.val(Cuestionario.oldElement.idpregunta);
	$("#cuestionario").val($.trim(Cuestionario.oldElement.cuestionario));
	$("#taskcuestionario").val('update');
	$('#btnaction_cuestionario_new').hide();
	$('#btnaction_cuestionario').html('Actualizar');
	$("#title_cuestionario").html('Actualizar Cuestionario');
	$("#modal_cuestionario").modal();
}
...
$('.btnaction').click(function () {
	var btnid=this.id;
	Cuestionario.cuestionario_form.data('bootstrapValidator').validate();
	if(Cuestionario.cuestionario_form.data('bootstrapValidator').isValid())
	{
		var cuestionario={};
		cuestionario.resp_correcta =0 ;
		if($("#resp_correcta").prop('checked'))
			cuestionario.resp_correcta =1;
		cuestionario.id_pregunta=Cuestionario.combobox_pregunta.val();
		cuestionario.id_respuesta=Cuestionario.combobox_respuesta.val();
		cuestionario.cuestionario = $("#cuestionario").val();
		var url=urlhome+"gestion/cuestionario/create";
		var title="Insertando elemento...";
		if($('#taskcuestionario').val()=="update")
		{
			var url=urlhome+"gestion/cuestionario/update";
			title="Actualizando elemento...";
			$('#btnaction_cuestionario_new').hide();
			cuestionario.idcuestionario=Cuestionario.oldElement.idcuestionario;

		}
		Noty('#2D8F3C', 'success', false, 'fa fa-spinner faa-spin animated', '', title,'center');
		if(btnid.indexOf('new')!=-1 && $('#taskcuestionario').val() != 'update')
			$.ajax({
				type: "POST",
				url:url,
				data:{
				...
				success:function(response){
					if(response.success)
					{
						...
						Noty('#2D8F3C','success',2000,'fa fa-thumbs-o-up','El cuestionario fue '+accion+' correctamente', message,'topRight');
					}
					else
					{
...
//Eliminar elemento
function delete_element_cuestionariobypreg(e) {
	var dataItem={};
	...
	$.MetroMessageBox({
			...
			if(a=="Aceptar")
			{
				...
				$.ajax({
					type: "POST",
					url:urlhome+"/gestion/cuestionario/deletebypreg",
					data:{
...
function resetcuestionarioForm() {
	$('#select2-id_respuesta-container').html('');
	Cuestionario.combobox_respuesta.append('<option selected="" value=""></option>').change();
	$('#select2-id_pregunta-container').html('');
	Cuestionario.combobox_pregunta.append('<option selected="" value=""></option>').change();
